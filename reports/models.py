...
class Appointment(models.Model):
    ...
    )  # Relación con terapeuta, puede ser nula (sin terapeuta asignado)
    patient = models.ForeignKey(
        Patient,
        on_delete=models.CASCADE
    )  # Relación con paciente, obligatorio
    ...
    payment = models.DecimalField(
        ...
        null=True,
        blank=True
    )  # Monto pagado, puede ser nulo
    payment_type = models.ForeignKey(
        PaymentType,
        on_delete=models.SET_NULL,
        null=True,
        blank=True
    )  # Tipo de pago usado, puede ser nulo
    deleted_at = models.DateTimeField(
        null=True,
        blank=True
    )  # Fecha y hora de eliminación lógica (soft delete), si aplica
